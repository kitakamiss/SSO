...
import com.hjrh.sso.client.handler.AppClientLogoutHandler;
...
public class SsoClientLogoutFilter extends BaseClientFilter{
	...
	private static  Logger logger = Logger.getLogger(SsoClientLogoutFilter.class.getName());
	 ...
	 */
	protected String appClientLogoutHandlerClass = "com.github.ebnew.sso.app.custom.ssoAppClientLogoutHandlerImpl";
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		try{
			//本地应用已经登录，则进行登出处理。
			if(session!=null && session.getAttribute(SsoClientFilter.USER_STATE_IN_SESSION_KEY)!=null){
				if(session.getAttribute(SsoClientFilter.USER_STATE_IN_SESSION_KEY)!=null){
					//清除session中的值。
