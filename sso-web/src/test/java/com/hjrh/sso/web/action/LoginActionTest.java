...
import com.hjrh.sso.core.exception.UsernameInvalidException;
...
import com.hjrh.sso.web.action.CredentialResolver;
import com.hjrh.sso.web.action.LoginAction;
import com.hjrh.sso.web.action.LoginResultToView;
import com.hjrh.sso.web.action.UsernamePasswordCredentialResolver;
...
public class LoginActionTest extends AbstractJUnit4SpringContextTests{
	...
	public void unitTestLogin() {
		...
		Assert.assertEquals("http://test.com/hello.jsp", request.getSession().getAttribute(WebConstants.sso_SERVICE_KEY_IN_SESSION));
	...
	public void integrationTestLogin() {
		...
		//测试模拟一个非法的已加密凭据值。
		Cookie cookie = new Cookie(WebConstants.sso_SERVER_ENCRYPTED_CREDENTIAL_COOKIE_KEY, "a invalid sso_SERVER_EC value");
		 ...
		 * 此时返回的地址重新定位到应用的入口地址，带上参数service和WebConstants.sso_CLIENT_ENCRYPTED_CREDENTIAL_COOKIE_KEY
		 * 其中参数service表示用户想要访问的地址，WebConstants.sso_CLIENT_ENCRYPTED_CREDENTIAL_COOKIE_KEY 表示应用中cookie
		 */
		request = new MockHttpServletRequest();
		response = new MockHttpServletResponse();
		...
		//设置cookie中合法的已认证凭据。
		String encCredential = "a valid sso_SERVER_EC value";
		cookie = new Cookie(WebConstants.sso_SERVER_ENCRYPTED_CREDENTIAL_COOKIE_KEY, encCredential);
		request.setCookies(cookie);
		mv = loginAction.login(request, response);
		Assert.assertNotNull(mv);
		//返回的视图类型是重定向视图。
		...
		Assert.assertEquals("http://test.com/loginAction.jsp?service=http://test.com/user.jsp&"+WebConstants.sso_CLIENT_ENCRYPTED_CREDENTIAL_COOKIE_KEY+"sso CLIENT ENCRYPTED CREDENTIAL COOKIE"
		...
		Assert.assertEquals(WebConstants.sso_SERVER_ENCRYPTED_CREDENTIAL_COOKIE_KEY, response.getCookies()[0].getName());
