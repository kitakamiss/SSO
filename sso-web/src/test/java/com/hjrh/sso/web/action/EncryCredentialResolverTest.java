...
import com.hjrh.sso.web.action.EncryCredentialResolver;
...
public class EncryCredentialResolverTest {
	...
	public void testResolveCredential(){
		...
		//测试存在cookie数组，但是有1个不为null的情况，且cookie名称为sso服务端写入的cookie值,但是cookie的值为Null.
		request = new MockHttpServletRequest();
		request.setCookies(new Cookie("C1", null), null, null, new Cookie(WebConstants.sso_SERVER_ENCRYPTED_CREDENTIAL_COOKIE_KEY, null));
		Assert.assertNull(resolver.resolveCredential(request));
		
		
		//测试存在cookie数组，但是有1个不为null的情况，且cookie名称为sso服务端写入的cookie值,但是cookie的值不是Null.
		request = new MockHttpServletRequest();
		request.setCookies(new Cookie("C1", null), null, null, new Cookie(WebConstants.sso_SERVER_ENCRYPTED_CREDENTIAL_COOKIE_KEY, cookie));
